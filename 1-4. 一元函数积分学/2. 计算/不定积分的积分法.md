...
题目：
-  [09. 一元函数积分学的计算](0.%20杂项/题目/09.%20一元函数积分学的计算.md#（20）)
...
> > ![](assets/不定积分的积分法/file-20260501085711469.png)
...
> [!example]  #秒杀技巧 🔥
> 
> **指数 × 三角函数的不定积分——行列式秒杀公式**
> 
> 对于 $\int e^{ax}\sin(bx)\,\mathrm{d}x$ 和 $\int e^{ax}\cos(bx)\,\mathrm{d}x$，不需要老老实实分部积分两次。记住下面这个行列式形式：
> 
...
> 
> $$\int e^{-x}\sin(nx)\,\mathrm{d}x = \frac{-e^{-x}\sin nx - ne^{-x}\cos nx}{(-1)^{2}+n^{2}} + C = -\frac{e^{-x}(\sin nx + n\cos nx)}{1+n^{2}} + C$$
...
![](assets/不定积分的积分法/file-20260502112250832.png)
