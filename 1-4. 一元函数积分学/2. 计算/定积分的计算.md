...
- $f(x)$ 连续，则： $$\int^{b}_{a} f(x) \mathrm{d}x = \int^{b}_{a} f(a+b-x) \mathrm{d}x$$ 称作**区间再现公式**。*证明 [[../../Excalidraw/例题/高数/第9讲例题#^vmfbaFam]]* ^0je46w
...
> 	- 偶数次为两倍的华理士公式，奇数次为 0（若被积函数为 $|\cos^n x|$，奇数次时不为 0，仍按偶数次的方式进行计算） #注意 
...
> 	- 偶数次为四倍的华理士公式，奇数次为 0（若被积函数为 $|\cos^n x|$，奇数次时不为 0，仍按偶数次的方式进行计算）
...
> [[../../Excalidraw/1000题/基础篇/高数/第14章#^C3YwBjUT]]
> 
