...
- $\int \frac {\mathrm {d} x}{\cos x} = \int \sec x \mathrm {d} x = \ln | \sec x + \tan x | + C=\ln |\sec x + \tan x| + C; \quad \star$ 
	- 证明：[09. 一元函数积分学的计算](0.%20杂项/题目/09.%20一元函数积分学的计算.md#（3）)
...
>
> **几何意义**（辅助理解）：$y=\sqrt{a^2-x^2}$ 是以原点为圆心、半径为 $a$ 的上半圆。定积分 $\int_0^x\sqrt{a^2-t^2}\,dt$ 表示半圆在 $[0,x]$ 与 $x$ 轴围成的面积。将该区域用从原点指向圆弧点 $(x,\sqrt{a^2-x^2})$ 的半径线分割为两部分：
