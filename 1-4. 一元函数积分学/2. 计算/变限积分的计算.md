...
> 
> 若被积函数中有**求导变量**，则必须通过恒等变形（变量代换等）将其 *移出被积函数* 才可以使用变限积分求导公式。
> 
...
3. $$\left\{\begin{matrix} f(x)T周期 \\ \int^{T}_{0} f(x) \mathrm{d}x = 0 \end{matrix}\right. \Rightarrow \int^{x}_{0} f(t) \mathrm{d}t ~T周期, $$
$$\left\{\begin{matrix} f(x)T周期 \\ \int^{T}_{a} f(x) \mathrm{d}x = 0 \end{matrix}\right. \Rightarrow \int^{x}_{a} f(t) \mathrm{d}t ~T周期, \forall a$$
	注意，上述内容是说，当满足前两个条件时，能得到**整个积分**是周期函数
	下面这个是**积分上下限**的的周期变化![定积分的计算](1-4.%20一元函数积分学/2.%20计算/定积分的计算.md#^mse3ty)
