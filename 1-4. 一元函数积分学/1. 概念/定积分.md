...
$$
![](assets/定积分/file-20260911215228590.png)

> [!cite]
> 5.38 02/13 DUL:/ S@Y.zt :2 pm 26 年数二神题，听不懂我吃屎（二重积分求极限）神题！好题！26 届数二最精彩的选择题！ 二重积分定义求极限，还结合了轮换对称性，真的很不错！# 考研 # 考研数学 # 考研数学凯哥 # 考研上岸 https://v.douyin.com/CdxVMU6X5gg/ 复制此链接，打开 Dou 音搜索，直接观看视频！
...
1. 若 $f(x)$ 在**闭区间**上**连续**，则积分存在；
	- 连续函数一定有[不定积分](1-4.%20一元函数积分学/1.%20概念/不定积分.md#^zje9ul)
...
> [!caution] #避坑
> 保号性**必须满足 $a<b$**。
> - 当 $a<b$ 时：$f(x) \ge 0 \Rightarrow \int_a^b f(x)\mathrm{d}x \ge 0$ ✓
> - 当 $a>b$ 时：$\int_a^b f(x)\mathrm{d}x = -\int_b^a f(x)\mathrm{d}x$，不等号方向**反转**，保号性不成立 ✗
> - 当 $a=b$ 时：$\int_a^a f(x)\mathrm{d}x = 0$
>
> **判断口诀**：看到 $f(x) \ge 0$ 想用保号性，先看积分方向——**下限小、上限大**才能直接用。
...
> 与[三角不等式](0.%20杂项/常用公式.md#三角不等式)类似 ^0vesv8
> **在证明积分等式与积分不等式中常用**
> [积分等式](../3.%20应用/2.%20积分等式与积分不等式/积分等式.md)
> [积分不等式](../3.%20应用/2.%20积分等式与积分不等式/积分不等式.md)
...
设 $f(x)$ 在闭区间连续，则在**闭区间**内至少存在一点 $\xi$ ，使得：
