...
> [!note] 
> 
> - 一元函数的极限只有**两种**趋近方式（正负），但在多元函数里面有**无数**种趋近方式（弯弯绕绕）；
> - #常用 若两种趋近方式得出的值 **不相等**，或某一路径极限值**不存在**，则极限**不存在**；
...
如果函数在**区域上每个点都连续**，则函数在区域上连续。

### 一元函数间断 ：
![](assets/基本概念/file-20260609164825794.png)
### 二元函数间断
![](assets/基本概念/file-20260609164842963.png)
...
如果 $z=f(x,y)$ 的两个二阶混合偏导数 $f''_{xy} ~,~ f''_{yx}$ 都在区域**连续**，则区域内： $$f''_{xy} = f''_{yx}$$ 即：**二阶混合偏导数在连续的条件下与求导的顺序无关**。
...
> [!note] 
> 联系一元微分：[微分的概念](../1-3.%20一元函数微分学/1.%20概念/微分的概念.md)
