...
| 非齐次解 $-$ 非齐次解 | 齐次解 |
| 齐次解 $+$ 非齐次特解 | 非齐次解 |
| 两个非齐次解直接相加 | 一般满足 $A\boldsymbol{x}=2\boldsymbol{b}$，不是原方程的解 |
...
> [!秒杀技巧] 求特解不要求线性无关，更不要求“长得漂亮”；自由变量取 $0$，能少算一笔是一笔。

> [!避坑] 求导出组时挡住增广列，求特解时再把增广列放回来；齐次、非齐次两套右端混着算，必炸。
...
且 $\boldsymbol{\eta}_1,\boldsymbol{\eta}_2,
\boldsymbol{\eta}_3$ 都是 $A\boldsymbol{x}=\boldsymbol{b}$ 的解，并给出
...
因此 $\boldsymbol{\alpha}_1,\boldsymbol{\alpha}_2,
\boldsymbol{\alpha}_3$ 的秩至多为 $2$。而四个向量的秩为 $3$，添入一个 $\boldsymbol{\alpha}_4$ 最多使秩增加 $1$，故
