...
**核心观察**:本题 $f$ 的两个位置**地位不对等**——1号位是**裸变量 $x$**,2号位是**函数 $e^x$**(占了"一个位置但装着一个函数")。所以画图时**右边那个位置要画成"带门"的格子**:对位置求导时**先把门关上**(只看"第几个位置"),**求完导才开门**看门后是 $e^x$。
...
| **记号** $z = f(u, v)$ | $f_1', f_2', f_{11}'', f_{12}''$ | 对**位置**求导, 与位置里是什么无关 |
| **记号** $z = f(x, y)$ | $f_x', f_y'$ | 对**自变量**求导(无复合时) |
